...
ROWS = [
    ...
    ("kv", "Learning", "SpringBoot, Rust, Java"),
    ("kv", "Goals", "Building scalable web apps"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "React, HTML, CSS, Tailwind, Bootstrap"),
    ("kv", "Backend", "Laravel, Node.js, Go, Express"),
    ("kv", "Languages", "Python, Go, Java, Rust, C, JS, PHP"),
    ("kv", "Databases", "MySQL, MongoDB"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Currently building '01-Blog' project"),
    ("bul", "Active full-stack developer & problem solver"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">mohamed@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">mohamed</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X+122}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
